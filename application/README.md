...
데스크톱 앱에서 **이 Flow 요약**을 누르면 편집 창의 미전송 내용을 저장한 뒤 OpenRouter로 현재 Flow를 요약합니다. API 키가 없으면 표시되는 [OpenRouter API 키 발급 페이지](https://openrouter.ai/keys)에서 키를 만든 뒤 앱에 등록합니다. 키는 Windows 자격 증명 관리자에 저장되며 선택된 Flow 내용은 OpenRouter로 전달됩니다.
...
저장 실패·API 키 오류·빈 응답은 화면에 표시합니다. 입력 크기는 UTF-8 JSON 기준 2MB까지입니다. 요약은 연결을 무방향 관계로 취급하며 블록 저장 순서를 인과 순서로 해석하지 않도록 요청합니다.

## 블록과 연결

- 새 Flow를 만들고 **블록 추가**를 누릅니다. 블록은 자동 연결되지 않습니다.
- **Ctrl+T**를 누르면 마지막 포인터 위치에 새 블록을 만듭니다. 텍스트 입력 중에는 작동하지 않습니다.
- 블록을 드래그해 이동합니다. 빈 공간 드래그는 영역 선택, 우클릭 드래그는 패닝입니다.
- 블록을 클릭해 선택합니다. 블록에 포커스를 두고 **Enter**를 누르거나 더블클릭하면 편집창이 해당 블록으로 전환되어 앞으로 옵니다.
